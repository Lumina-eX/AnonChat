...
- `015_invite_expiration_system.sql` (new)
...
- `scripts/015_invite_expiration_system.sql` implements comprehensive invite expiration support including:
  - Time-based expiration validation with `is_invite_time_expired()` function
  - Usage-based expiration validation with `is_invite_usage_expired()` function
  - Invite expiration audit logging via `invite_expiration_logs` table
  - Automatic cleanup functions (`cleanup_expired_invites()`) to identify and log expired codes
  - Status checking function (`get_invite_status()`) for real-time expiration information
  - Support for three expiration types: `time_expired`, `usage_limit_reached`, and `manually_invalidated`
  - New audit event types: `invite_expired` and `invite_limit_reached` for blockchain audit trail
